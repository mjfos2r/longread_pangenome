...
def get_input_files(in_dir, ext):
    ...
    print(f'Input files found: {get_count(files)}')
    return files
...
def queue_job(futures, function, command):
    futures.append(executor.submit(function, command))
...
def main():
    ...
    parser.add_argument('--db',    required=True, type=str, help='path to directory containing all blast databases to use in analysis')
