...
def main():
    if len(sys.argv) != 4:
        print("Usage: python script.py homology_network.tsv input.fasta output_dir")
        sys.exit(1)
    
    network_file = pathlib.Path(sys.argv[1])
    fasta_file   = pathlib.Path(sys.argv[2])
    output_dir   = pathlib.Path(sys.argv[3])
    prefix = network_file.stem
    # Create and save matrix
    matrix_df, contig_lengths = create_alignment_matrix(network_file, fasta_file)
    matrix_df.to_csv(output_dir.joinpath(f'{prefix}_alignment_matrix.tsv'), sep='\t')
