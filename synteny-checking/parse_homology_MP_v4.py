...
def get_coverage(length, coords):
    ...
    for start, end, ident in coords:
        ...
        identities += (end - start + 1) * float(ident/100) #float(1/ident) here's why that was broken i think. I cannot believe I fucked this up lol.
...
def process_alignment_nucl(alignment_file, genbank_paths):
...
def process_alignment_gene(alignment_file, genbank_paths):
    asm1_id, asm2_id = os.path.basename(os.path.dirname(alignment_file)).split('_vs_')
    ...
    if not check_tsv_content(alignment_file):
        # File is empty, return a structured result with placeholder data
        placeholder_row = [
            asm1_id,  # asm1_id
            "N/A",    # asm1_locus_tag
            "N/A",    # asm1_product
        ...
        return (False, f'NO HOMOLOGY BETWEEN {asm1_id} and {asm2_id}!', [placeholder_row], alignment_file)

    alignments = parse_alignment(alignment_file)
    gene_homology_rows = get_gene_homology(alignments, asm1_record, asm2_record)

    return (True, f'Parsed gene homology between {asm1_id} and {asm2_id}!', gene_homology_rows, None)

def get_gene_homology(alignments, asm1_record, asm2_record):
    gene_homology_rows = []
...
def find_and_process_alignments(directory, genbank_paths, num_cpus, progress, expected_count, max_queue_size, analysis_type):
    ...
    if analysis_type == 'nucl':
        ...
        return gene_homology_rows, processed_count, elapsed_time, empty_alignment_files
...
def generate_shared_gene_matrix(gene_homology_rows, identity_cutoff):
    ...
    for row in gene_homology_rows[1:]:  # Skip header row
        if float(row[-1]) >= identity_cutoff:
            ...
            if asm1_idx != asm2_idx:
                shared_gene_matrix[asm2_idx, asm1_idx] += 1  # Symmetric matrix, but don't double-count diagonal
...
def write_output_files_gene(output_dir, gene_homology_rows, progress, version, min_identity, identity_step, empty_alignment_files):
    ...
    os.makedirs(f'{output_dir}/matrices')
    for identity_cutoff in range(int(min_identity), 101, int(identity_step)):
        ...
        shared_gene_matrix = generate_shared_gene_matrix([header] + filtered_rows, identity_cutoff)
    ...
    try:
        ...
        with Live(layout, refresh_per_second=10) as live:
            ...
            if args.analysis_type == 'nucl':
                ...
                gene_homology_rows, num_aln_files, elapsed_time, empty_alignment_files = find_and_process_alignments(
                    args.alignments_dir, genbank_paths, args.cores, progress, expected_alignments, max_queue_size, args.analysis_type
                    )
            ...
            if args.analysis_type == 'nucl':
                ...
                write_output_files_gene(
                    args.output_dir, gene_homology_rows, progress, args.version, args.min_identity, args.identity_step, empty_alignment_files
                )
