...
rule all:
    input:
        f'{analysis_dir}/pangenome/v3/roary_v3_1/core_gene_alignment.aln',
        f'{analysis_dir}/pangenome/v3/tree/v3/fasttree_roary_core_v3.newick',
        f'{analysis_dir}/pangenome/v3/tree/v3/RAxML_bestTree.roary_v3_1_gtrgam',
...
rule Roary:
    ...
    output:
        outdir = directory(f'{analysis_dir}/pangenome/v3/roary_v3_1/'),
        outfile = f'{analysis_dir}/pangenome/v3/roary_v3_1/core_gene_alignment.aln',
        outasm = f'{analysis_dir}/pangenome/v3/roary_v3_1/pan_genome_reference.fa',
    threads: 15
...
rule RAxML:
    ...
    output:
        ...
        outfile = f'{analysis_dir}/pangenome/v3/tree/v3/RAxML_bestTree.roary_v3_1_gtrgam'
    threads: 15
    conda: "raxml"
    message: "running RAxML on the roary output ==>> {input}"
    log: f'{analysis_dir}/pangenome/v3/logs/RAxML_log.txt'
    shell:
        "raxmlHPC -m GTRGAMMA -p 71235 -s {input} -n roary_v3_1_gtrgam -w {output.outdir} >{log} 2>&1"

rule bakta:
    input: f'{analysis_dir}/pangenome/v3/roary_v3_1/pan_genome_reference.fa'
    output:
