...
rule bakta:
    ...
    shell:
        "bakta --db {params.bakta_params[db]} {params.bakta_params[gram]} --skip-trna --keep-contig-headers --genus Borrelia --species burgdorferi --strain pan-sensu-stricto --threads {threads} {params.bakta_params[opts]} --force --output {output.outdir} --prefix roary_pangene_v2 {input} 2>&1 >{log}"
