...
def get_homologies(alignments, asm1_dict, asm2_dict):
    ...
    for alignment in alignments: # iterate through each alignment for this particular comparison.
        ...
        ##########################################################################################
        # Be sure to change ID back to name for alternate analyses. {{TO-DO: FIX SCUFFED PARSER}}
        asm1_length          = len(asm1_dict[asm1_name].seq)
        ##########################################################################################
        asm2_id              = alignment['REF_ID']
        asm2_name            = alignment['REF_NAME'] #.replace("0000", "_").replace("_0", "_")
        ...
        if asm2_id not in genes_dict[asm1_name]: # if the ref is not in the contig dict, add it and set val to an empty list.
        ...
        alignment_dict = {
                        'asm_aln': {
                                ...
                                #'len_homology': float((asm1_end-asm1_start)*asm2_identity),
                                'percent_cov': int(),
                                'features': asm1_genes,
                            },
                        'ref_aln': {
                                ...
                                'percent_cov': int(),
                                #'len_homology': float((asm2_end-asm2_start)*asm2_identity),
                                'features': asm2_genes,
...
def make_table_for_asm(genes_dict):
    ...
    for asm1_id in genes_dict.keys():
        ...
        for asm2_id in genes_dict[asm1_id].keys():
            assembly1_id = genes_dict[asm1_id]['assembly_id']
            asm1_region_genes = []
            asm2_region_genes = []
            if asm2_id != 'contig_length' and asm2_id != 'assembly_id': # see above to-do re: separate dict just for len/assembly stats.
                ...
                asm2_coords = [(cov['asm_aln']['start'],cov['asm_aln']['end'], cov['ref_aln']['aln_identity']) for cov in genes_dict[asm1_id][asm2_id]]
                asm1_aln_coords = [(cov['ref_aln']['start'],cov['ref_aln']['end'], cov['ref_aln']['aln_identity']) for cov in genes_dict[asm1_id][asm2_id]]
                asm2_len = genes_dict[asm1_id][asm2_id][0]['ref_aln']['ref_length']
                asm1_len_homology = 0
                asm2_len_homology = 0
                assembly2_id = genes_dict[asm1_id][asm2_id][0]['ref_aln']['ref_id']
                for index, item in enumerate(genes_dict[asm1_id][asm2_id]):
                    #asm1_len_homology = item['asm_aln']['len_homology']
                    #asm2_len_homology = item['ref_aln']['len_homology']
                ...
                asm1_cov, asm1_identities, asm1_covered_positions= get_coverage(asm1_len, asm1_aln_coords)
                asm2_cov, asm2_identities, asm2_covered_positions = get_coverage(asm2_len, asm2_coords)
                #print(contig_cov, aln_coords)
                output_row = [assembly1_id, asm1_id, asm1_len,
                              assembly2_id, asm2_id, asm2_len,
                              f'{asm1_cov:.2f}', len(asm1_covered_positions), 
                              f'{asm2_cov:.2f}', len(asm2_covered_positions),
    ...
    for contig in genes_dict.keys():
        ...
        for aln in genes_dict[contig].keys():
            if aln != 'contig_length' and aln != 'assembly_id':
                asm_coords = [(cov['asm_aln']['start'],cov['asm_aln']['end'], cov['ref_aln']['aln_identity']) for cov in genes_dict[contig][aln]]
                ref_coords = [(cov['ref_aln']['start'],cov['ref_aln']['end'], cov['ref_aln']['aln_identity']) for cov in genes_dict[contig][aln]]
                ref_len = genes_dict[contig][aln][0]['ref_aln']['ref_length']
                contig_cov, asm1_identities, asm1_covered_positions = get_coverage(contig_len, asm_coords)
                ref_cov, asm2_identities, asm2_covered_positions = get_coverage(ref_len, ref_coords)
...
def parallel_parse(cpus, alignment_files, annotations_dir):
    ...
    detailed_header_row = ['assembly1_id', 'asm1_id', 'asm1_len', 
                           'assembly2_id', 'asm2_id', 'asm2_len', 
                           'asm1_cov', 'asm1_identities', 
                           'asm2_cov', 'asm2_identities', 
                           'asm1_genes', 'asm2_genes']
    
    simple_header = ['contig_id','contig_len','total_contig_coverage','list_of_alignments(asm2:percent_cov_of_contig)']
...
def main():
    ...
    if mode == 'ava':
        if skip_parse:
            ...
            print("beginning results parsing. please stand by!")
